...
async fn main() {
    ...
    let app = Router::new()
        ...
        .nest_service(
            "/",
            ServeDir::new("/app/web")
                .append_index_html_on_directories(true)
                .fallback(ServeFile::new("/app/web/index.html")),
        )
        .with_state(state);
...
async fn codex_runtime(State(state): State<AppState>) -> Json<CodexRuntimeResponse> {
    ...
    let version_output = tokio::time::timeout(
        ...
        Command::new(state.codex_bin.as_str())
            .arg("--version")
            .output(),
    )
    .await;
    ...
    Json(CodexRuntimeResponse {
        installed,
        version,
        mode: state.codex_mode.as_str().to_owned(),
        authenticated,
    })
}
...
pub(crate) async fn connect(state: &AppState) -> Result<Client, ApiError> {
    let (client, connection) = tokio_postgres::connect(state.database_url.as_str(), NoTls)
        .await
...
/// 将数据库行映射为项目响应。
fn project_json(row: &Row) -> Value {
    json!({ "id": row.get::<_, String>(0), "name": row.get::<_, String>(1), "description": row.get::<_, String>(2), "status": row.get::<_, String>(3), "created_at": row.get::<_, String>(4), "updated_at": row.get::<_, String>(5) })
}
...
fn task_json(row: &Row) -> Value {
    json!({
        ...
        "source_type": row.get::<_, String>(16),
        "source_workflow_run_id": row.get::<_, Option<String>>(17),
        "source_node_run_id": row.get::<_, Option<String>>(18),
        "workflow_name": row.get::<_, Option<String>>(19)
    })
}
...
async fn list_projects(State(state): State<AppState>) -> Result<Json<Value>, ApiError> {
    ...
    Ok(Json(
        json!({ "items": rows.iter().map(project_json).collect::<Vec<_>>(), "next_cursor": null }),
    ))
}
...
async fn update_project(
    ...
    if name.is_none() && description.is_none() {
        return Err(ApiError::invalid("name or description is required"));
    }
    let client = connect(&state).await?;
    ...
    let rows = client
        .query(
            concat!(
                ...
                "to_char(t.updated_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), t.collaboration_status, t.source_type, ",
                "t.source_workflow_run_id, t.source_node_run_id, t.workflow_name FROM tasks t ",
        ...
        .map_err(ApiError::database)?;
    Ok(Json(
        json!({ "items": rows.iter().map(task_json).collect::<Vec<_>>(), "next_cursor": null }),
    ))
}
...
async fn create_task(
    ...
    if title.len() > 500 {
        return Err(ApiError::invalid("title is too long"));
    }
    let description = body
        .get("description")
        .and_then(Value::as_str)
    ...
    let requires = body
        .get("requires_plan_confirmation")
    ...
    let row = transaction
        .query_one(
            concat!(
                ...
                "to_char(created_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), to_char(updated_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), ",
                "collaboration_status, source_type, source_workflow_run_id, source_node_run_id, workflow_name"
            ),
    ...
    let row = client
        .query_opt(
            concat!(
                ...
                "to_char(t.created_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), ",
                "to_char(t.updated_at, 'YYYY-MM-DD\"T\"HH24:MI:SS.MS\"Z\"'), t.collaboration_status, t.source_type, ",
                "t.source_workflow_run_id, t.source_node_run_id, t.workflow_name FROM tasks t WHERE t.id = $1"
            ),
            &[&task_id],
        )
        .await
...
async fn update_task(
    ...
    if title.is_none() && description.is_none() && requires.is_none() && progress.is_none() {
        return Err(ApiError::invalid("no mutable fields"));
    }
...
async fn transition_task(
    ...
    let target_stage = xiexu_domain::BoardStage::parse(&target)
        .ok_or_else(|| ApiError::invalid("invalid target_stage"))?;
    ...
    let from_stage = xiexu_domain::BoardStage::parse(&from)
        .ok_or_else(|| ApiError::invalid("stored task stage is invalid"))?;
    if !xiexu_domain::is_valid_transition(from_stage, target_stage) {
        return Err(ApiError::conflict(format!(
            "invalid transition: {from} -> {target}"
        )));
    }
    let requires = current.get::<_, bool>(1);
    let plan_status = if target_stage == xiexu_domain::BoardStage::PlanReview {
        "reviewing"
    } else if target_stage == xiexu_domain::BoardStage::Todo && requires {
        "pending_generation"
    } else if target_stage == xiexu_domain::BoardStage::InProgress {
        "approved"
    } else {
        "not_required"
    };
    let execution_status = if target_stage == xiexu_domain::BoardStage::InProgress {
        "queued"
    } else {
        "idle"
    };
    ...
    if target_stage == xiexu_domain::BoardStage::Done {
        ...
        project_context::enqueue_parent_document_refresh(
            &transaction,
            &task_id,
            &project_id,
            next_revision,
...
async fn list_events(
    ...
    Ok(Json(
        json!({ "items": rows.iter().map(|row| json!({ "id": row.get::<_, i64>(0), "event_type": row.get::<_, String>(1), "actor_type": row.get::<_, String>(2), "actor_id": row.get::<_, String>(3), "before_data": row.get::<_, Option<Value>>(4), "after_data": row.get::<_, Option<Value>>(5), "event_data": row.get::<_, Value>(6), "created_at": row.get::<_, String>(7) })).collect::<Vec<_>>() }),
    ))
}

/// 查询任务关联的执行作业、尝试、事件和输出，供运行记录页面复用。
async fn list_execution(
    State(state): State<AppState>,
    Path(task_id): Path<String>,
) -> Result<Json<Value>, ApiError> {
    let client = connect(&state).await?;
    if client
        .query_opt("SELECT id FROM tasks WHERE id = $1", &[&task_id])
        .await
        .map_err(ApiError::database)?
        .is_none()
    {
